...
const BlImage: React.FC<IProps> = (props: IProps) => {
  const { src } = props;
  const [hash, setHash] = useState('LRQ0XHWB?b%M~qofIURjWBt7j[M{');
  ...
  useEffect(() => {
    (async function () {
      setHash(await encodeImageToBlurhash(src));
    })();
  }, []);

  return (
    <>
      <Blurhash hash={hash} className={'blurhash-hash absolute top-0 left-0 z-0 !w-[100%] !h-[100%] grayscale'} />
      <motion.img
